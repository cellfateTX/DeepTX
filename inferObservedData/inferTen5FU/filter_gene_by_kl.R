...
result_dir = "result/Ten5FU"
data_dir = "data/Ten5FU"

# load data dmso
scRNA_dmso = read.table(sprintf("%s/doseZero_norm_filter.csv",data_dir),header=T, sep = ',')

ssa_dmso_dir =sprintf("%s/doseZeroSSADistribution/",result_dir)
...
write.csv(dmso_kl_df, sprintf("%s/doseZero_kl.csv",result_dir),quote = F,row.names=FALSE) 

# load data IdU
scRNA_IdU = read.table(sprintf("%s/doseTen_norm_filter.csv",data_dir),header=T, sep = ',')
ssa_IdU_dir = ssa_dmso_dir =sprintf("%s/doseTenSSADistribution/",result_dir)
...
write.csv(IdU_kl_df, sprintf("%s/doseTen_kl.csv",result_dir),quote = F,row.names=FALSE) 

gene_estimated_matrix_idu = read.csv(file = sprintf("%s/doseTen_estimated_model_stats_prob.csv",result_dir), row.names =
                                       6)
gene_estimated_matrix_dmso = read.csv(file = sprintf("%s/doseZero_estimated_model_stats_prob.csv",result_dir), row.names =
                                        6)
